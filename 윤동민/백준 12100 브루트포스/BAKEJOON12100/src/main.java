...
//                                    if (nextIndex+1 < Main.size) { row = nextIndex + 1; }
                                    if (nextIndex+1 < Main.size) { row = nextIndex; }
                                    break;
                                }
                            } else {
                                if (nextIndex == Main.size-1) {
                                    currentBoard[currentSuccessIndex++][j] = currentBoard[row][j];
                                    break;
                                }
                            }
                ...
                for(int k = currentSuccessIndex; k < Main.size; k++) { currentBoard[k][j] = 0; }
            }
        } else if(direction == "down") {
            for(int j = 0; j < Main.size; j++) {
                int currentSuccessIndex = Main.size-1;
                int row = Main.size-1;
                while (row >= 0) {
...
//                                    if (nextIndex-1 >= 0) { row = nextIndex - 1; }
            ...
            for(int i = 0; i < Main.size; i++) {
                ...
                int col = 0;
                while (col < Main.size) {
                    if(currentBoard[i][col] != 0 && col != Main.size-1) {
                        int nextIndex = col+1;
                        while (nextIndex < Main.size) {
...
//                                    if (nextIndex+1 < Main.size) { col = nextIndex + 1; }
                                    if (nextIndex+1 < Main.size) { col = nextIndex; }
                                    break;
                                }
                            } else {
                                if (nextIndex == Main.size-1) {
                    ...
                    if (col == Main.size-1) { currentBoard[i][currentSuccessIndex++] = currentBoard[i][col]; }
                    col++;
                }

                for(int k = currentSuccessIndex; k < Main.size; k++) { currentBoard[i][k] = 0; }
            }
        } else {
            for(int i = 0; i < Main.size; i++) {
                int currentSuccessIndex = Main.size-1;
